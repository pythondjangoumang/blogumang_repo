...
def post_detail_view(request,year,month,day,post):
    post=get_object_or_404(post_model,slug=post,status='published',publish__year=year,publish__month=month,publish__day=day)
...
def mail_send_view(request,id):
    post=get_object_or_404(post_model,id=id,status='published')
    sent=False
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            send_mail(subject,message,'umang.master92@gmail.com',[cd['to']])
            sent=True
    else:
        form=EmailSendForm()
